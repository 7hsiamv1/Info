...
def get_account_credentials(region: str) -> str:
    ...
    if r == "IND":
        ...
        return "uid=4411158804&password=ARMY_887TC_BY_7H_SIAM_VO4PP"
...
def get_account_info():
    ...
    # রিজিয়ন না দিলে ডিফল্ট 'BD' কাজ করবে
    ...
    try:
        ...
        # আপনার চাহিদা অনুযায়ী আউটপুট ফরম্যাট
        final_response = {
            "AccountInfo": {
                "AccountLevel": basic_info.get("level", 0),
                "AccountRegion": basic_info.get("region", region),
                "ReleaseVersion": RELEASEVERSION
            }
        }
        ...
        return jsonify({"error": f"Failed to retrieve data for UID {uid}.", "details": str(e)}), 500
...
def background_refresh_worker():
    """ Runs the periodic token refresh in a separate thread so it doesn't block Flask """
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    loop.run_until_complete(refresh_tokens_periodically())

if __name__ == '__main__':
    # Initialize tokens once before the server starts
    asyncio.run(initialize_tokens())
    
    # Start the background thread for automatic token refreshing
